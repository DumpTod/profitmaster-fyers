...
# ========================================
# TOKEN MANAGEMENT (Option B: Browser-Based Only)
# ========================================
...
def load_token():
    try:
        ...
        print(f"⚠ No token file found - please login via /refresh")


def init_fyers():
    if not token_data['access_token']:
        print("✗ init_fyers: No access token available - visit /refresh to login")
        return None
    try:
...
# Load token on startup (if exists)
load_token()
...
@app.route('/refresh')
def refresh_token():
    """Redirect to Fyers login page - user enters TOTP here"""
    auth_url = (
...
def set_token():
    ...
    if not access_token:
        ...
        <p style="color:#aaa;margin-bottom:20px">Login via /refresh first, or paste tokens manually below.</p>
        
        <div style="background:#1a2a4a;padding:20px;border-radius:8px;margin-bottom:20px">
            <h3 style="color:#22c55e;margin-bottom:10px">📋 Quick Steps:</h3>
            <ol style="color:#aaa;line-height:1.8">
                <li>Click "🔑 Login via Fyers" button below</li>
                <li>Enter your Fyers username & password</li>
                <li>Enter TOTP when prompted (Google Authenticator code)</li>
                <li>You'll be redirected back here automatically!</li>
            </ol>
        </div>

        <p style="margin-bottom:10px"><strong>OR paste manually:</strong></p>
        ...
        <p style="margin-top:30px">
            <a href="/refresh" style="color:#22c55e;text-decoration:none;padding:12px 24px;background:#166534;border-radius:6px;display:inline-block;font-weight:600">🔑 Login via Fyers (Recommended)</a>
        </p>
        </body></html>
        '''
    ...
    <h1 style="font-size:48px;margin-bottom:20px">✅ Success!</h1>
    <p style="font-size:18px;color:#22c55e;margin-bottom:10px">Token Saved Successfully!</p>
    <p style="color:#aaa">{refresh_msg}</p>
    <p style="color:#aaa;margin-top:10px">Time: {datetime.now(IST).strftime('%d %b %Y %H:%M:%S IST')}</p>
    <p style="margin-top:30px">
        <a href="/" style="color:#22c55e;text-decoration:none;padding:12px 30px;background:#166534;border-radius:6px;display:inline-block;font-size:16px;font-weight:600">📊 Go to Scanner →</a>
    </p>
    </body></html>
    '''
...
def trigger_auto_refresh():
    """Option B: Disabled - use /refresh instead"""
    return jsonify({
        'status': 'info',
        'message': 'Auto-refresh disabled (Option B mode). Please visit /refresh to login manually.',
        'token_status': 'active' if token_data['access_token'] else 'expired',
...
def generate_signals():
    ...
    for symbol, config in SCANNER_CONFIG.items():
        try:
            ...
            # Last 200 candles (captures entire trading day + buffer)
            if len(df) >= 200:
...
@app.route('/')
def home():
    token_status = '✅ Token Active' if token_data['access_token'] else '🔴 Token Expired'
    token_time = token_data.get('token_time', 'Never')
    
    login_btn = ''
    if not token_data['access_token']:
        login_btn = '<p style="margin-top:20px"><a href="/refresh" style="color:#f59e0b;text-decoration:none;padding:12px 24px;background:#78350f;border-radius:6px;display:inline-block;font-weight:600;font-size:14px">🔑 Click Here to Login (Takes 10 seconds)</a></p>'
    ...
    <p style="color:#aaa">ATR Trailing Stop — NIFTY & BANKNIFTY</p>
    <div style="background:#1a2a4a;padding:20px;margin:20px auto;max-width:600px;border-radius:8px">
        ...
        <p style="margin:10px"><strong>Last Login:</strong> {token_time}</p>
        <p style="margin:10px"><strong>Server Time:</strong> {datetime.now(IST).strftime('%d %b %Y %H:%M:%S IST')}</p>
        <p style="margin:10px"><strong>Keep-Alive:</strong> <span style="color:#22c55e">✅ Active (every 14 min)</span></p>
    </div>
    {login_btn}
    <p style="margin-top:20px"><a href="/api/signals" style="color:#22d3ee;text-decoration:none;padding:10px 20px;background:#164e63;border-radius:6px;display:inline-block;margin:5px">📊 Get Signals</a></p>
    </body></html>
    '''
...
if __name__ == '__main__':
    ...
    print(f"{'='*60}")
    print(f"Port: {port}")
    print(f"Token Status: {'✅ Active' if token_data['access_token'] else '🔴 Not Set - Visit /refresh'}")
    print(f"Keep-Alive: ✓ Every 14 minutes")
    print(f"Login Method: Browser-based (Option B)")
