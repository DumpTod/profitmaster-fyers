...
# ========================================
# TOKEN MANAGEMENT (ORIGINAL WORKING VERSION)
# ========================================
...
# ========================================
# SIGNAL GENERATION (WITH FIXES APPLIED)
# ========================================
def generate_signals():
    ...
    for symbol, config in SCANNER_CONFIG.items():
        try:
            ...
            # ✅ FIX #1: Changed days=5 to days=90
            ...
            # ✅ FIX #2: Changed from today-only to last 200 candles
            if len(df) >= 200:
                scan_df = df.tail(200).copy()
                print(f"✓ Scanning last 200 candles for {symbol} (was: today-only)")
            else:
                scan_df = df.copy()
...
def keep_alive_ping():
    while True:
        try:
            ...
            print(f"🔄 Keep-alive ping at {datetime.now(IST).strftime('%H:%M:%S IST')}")
        except Exception as e:
